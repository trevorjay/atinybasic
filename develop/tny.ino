...
int load_nearest_line(int num) {
  ...
  for (unsigned int i = 0; i < MAX_POS ; i++) {
    card.readData(((i*2)/512),(i % 256)*2,2,(uint8_t*) &line);
    if (line < num) continue;
    card.readData(128+((i*64)/512),(i % 8)*64,64,(uint8_t*) lbuf);
    card.readEnd();
    cur = 0;
    return read_number(&cur,lbuf);
  }
...
x17:
   if (!il_tst(")"))    goto f2;  
        ...
        goto            call_rtn;
pl:
   if (!il_tst("PLAY(")) goto rn;
        il_call(X21);   goto expr;
x21:
   if (!il_tst(","))    goto f2;  
        il_call(X22);   goto expr;
x22:
   if (!il_tst(")"))    goto f2;  
        il_ply();
        goto            call_rtn;
